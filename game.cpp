        ...
        if(IsKeyDown(KEY_S)){
            ...
            for (int i = 0; i < shell_num; i++){
                if (!shell[i].active && fire_rate % 40 == 0){
                    ...
                    PlaySound(shotgun);
                    shell[i].rec.x = marineAnim.rec.x + marineAnim.rec.height/2;
                    shell[i].rec.y = marineAnim.rec.y;
                    shell[i].active = true;
                    break;
                }
            }
